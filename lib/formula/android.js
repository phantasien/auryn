...
android.install = function (done) {
  config.getAll(function (err, cfg) {
    ...
    var env = {
      CC: path.join(ndkPrebuiltPath, 'arm-linux-androideabi-gcc'),
      CXX: path.join(ndkPrebuiltPath, 'arm-linux-androideabi-g++')
    }

    mkdirp(cfg.falkor.base, function () {
      var c = child.execFile(gypScript, args, {cwd: cfg.falkor.base, env: env}, function (err, stdout, stderr) {
        console.log(err);
      });
