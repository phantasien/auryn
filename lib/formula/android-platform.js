...
androidPlatform.validate = function (done) {
  config.getAll(function (err, cfg) {
    fs.exists(path.join(cfg.android.sdk, 'platforms', 'android-' + cfg.android.platform), function (exists) {
...
androidPlatform.install = function (done) {
  config.getAll(function (err, cfg) {
    ...
    var installer = child.spawn(androidBin, args);
    var bot = streamify();
    ...
    installer.stdout.pipe(bot);
    installer.stdout.on('end', done);
  });
};